...
function db_connect(){
    ...
    if (!$link) {
        # code...
    $config = array_merge(['host'=>'127.0.0.1','user'=>'root','pass'=>'admin','dbname'=>'photoalbum','port'=>3306],config('DB_CONNECT'));
    if (!$link = call_user_finc_arry('mysql_connect',$config)) {
        # code...
...
function db_fetch_row($sql,$type = '',array $data = []){
    $stmt = db_query($sql,$type,$data);
    return msyqli_fetch_assoc(mysqli_stmt_get_result($stmt));
}
function db_exec($sql,$type = '',array $data = []){
    $stmt = db_query($sql,$type,$data);
    return (strtoupper(substr(trim($sql),0,6))=='INSERT')?
        mysqli_stmt_insert_id($stmt) : mysqli_stmt_affected_rows($stmt);
}
